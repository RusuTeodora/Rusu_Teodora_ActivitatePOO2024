...
private:
	Magazin magazin;//are mai multe m
	Cofetarie *cofetarie;//O singura cofetarie pe care o retinem prin pointer
	int nrMasiniParcate;
...
public:
	...
	Magazin getMagazin() {
		return this->magazin;//se apleaza constructorul de copiere din Magazin
	}
	...
	}// apelam constructorii deja facuti in clase

	//CONSTRUCTOR CU PARAMETRII

	ComplexComercial(Magazin mag, Cofetarie* cof, int nrMasini, Masina* masini) {
		this->magazin = mag;//mag exista, this->magazin exista=>se apeleaza op = de la Magazin
		this->cofetarie = new Cofetarie(*cof);//atribuire de adrese
		//asa se obtine obiectul 
		//am alocat spatiu cu new
		//daca folosim () avem un singur obiect
		//apelez constructorul de copiere din Cofetarie
		this->nrMasiniParcate = nrMasini;
		this->masiniParcate = new Masina[nrMasini];
		//ca sa le parcurg pe toate
		for (int i = 0; i < nrMasini; i++) {
			this->masiniParcate[i] = masini[i];//iau fiecare masina si o pun in vectorul asta
			//apelez op = din clasa masina
		}
	}
	...
	friend ostream& operator<<(ostream& output, const ComplexComercial& cc) {
		output << "Magazinul din complexul comercial:" << endl;
		output << cc.magazin << endl;//se apeleaza op << din clasa magazin
		output << "Cofetaria din complex este:" << endl;
		output << *cc.cofetarie << endl;//se apleleaza op << din clasa Cofetarie
		//trebuie dereferentiata cu * altfel arata adresa 
		output << "Avem:" << cc.nrMasiniParcate << "masini parcate in complex" << endl;
		for (int i = 0; i < cc.nrMasiniParcate; i++) {
			output << cc.masiniParcate[i] << endl;
		}
		output << endl;
		return output;
	}

	//OPERATOR -- post decrementare (returneaza forma nemodificata)

	ComplexComercial operator--() {//forma prefixata
		//va returna val modificata
		if (this->nrMasiniParcate > 0) {
			this->nrMasiniParcate--;
			Masina* aux = new Masina[this->nrMasiniParcate];//se apeleaza constructorul  din Masina 
			for (int i = 0; i < nrMasiniParcate; i++) {
				aux[i] = this->masiniParcate[i];//se apleleaza op = din clasa Masina
			}
			delete[]this->masiniParcate;//se apeleaza destructorul din Masina de cate masini aveam inainte adica de 5 ori
			//
			this->masiniParcate = aux;//in aux raman doar cele 4
			//se apeleaza op = pt pointer
		}
		return *this;
	}

	//OPERATOR -- predrecrementare (returneaza forma modificata)

	ComplexComercial operator--(int) {
		ComplexComercial cc = *this;//se apeleaza constructorul de copiere sin ComplexComercial
		--(*this);// aplez ce am scris mai sus
		return cc;
	}
...
void main() {
	...
	ComplexComercial cc2(magazin,c,5,masini);//apelare constructor

	cout << cc2;//apel <<

	cc2--;
	--cc2;//sa adauge o noua masina parcata

	cout << cc2;

	cc1 = cc2--;//returneaza tot 5 masini
